...
(
	...
	CONSTRAINT FK_LBA_Partidos_LBA_Equipos1 FOREIGN KEY (id_Local) REFERENCES LBA_Equipos (Codigo) ON DELETE NO ACTION ON UPDATE CASCADE,
	CONSTRAINT FK_LBA_Partidos_LBA_Equipos2 FOREIGN KEY (id_Visitante) REFERENCES LBA_Equipos (Codigo) ON DELETE NO ACTION ON UPDATE NO ACTION
)
...
ALTER TABLE LBA_Jugadores WITH NOCHECK ADD CONSTRAINT CK_EdadRequerida CHECK ((YEAR(CURRENT_TIMESTAMP - CAST (FechaNac AS DATETIME)) - 1900) BETWEEN 15 AND 50)
...
ALTER TABLE LBA_Equipos WITH NOCHECK ADD CONSTRAINT CK_Fecha_Fundacion CHECK (Fecha_Fundacion < CURRENT_TIMESTAMP)
ALTER TABLE LBA_Juega WITH NOCHECK ADD CONSTRAINT CK_Minutos CHECK (Minutos BETWEEN 0 AND 40)
ALTER TABLE LBA_Juega WITH NOCHECK ADD CONSTRAINT CK_Tiros CHECK (Tiros1 >= Puntos1 AND Tiros2 >= (Puntos2 / 2) AND Tiros3 >= (Puntos3 / 3))
